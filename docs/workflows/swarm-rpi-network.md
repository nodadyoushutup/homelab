...
Host fstab is separate from Swarm stack NFS local volumes. After changing host mounts, add the same flags to live `swarm_nfs_volume_o_rw` / `swarm_nfs_volume_o_ro` in `<homelab>/.config/terraform/providers/nfs.tfvars` (see `terraform/providers/nfs.tfvars.example`), then re-apply stacks that mount NFS.
