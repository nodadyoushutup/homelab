...
- [x] Validate Alloy config syntax.
- [x] Apply Terraform stack and verify Alloy tasks are healthy on all Swarm nodes.
- [x] Verify logs are ingested into Loki from Alloy.
...
- `docker -H ssh://swarm-cp-0.local service ls` shows `alloy` at `6/6` in global mode.
- `docker -H ssh://swarm-cp-0.local service ps alloy --no-trunc` shows one running task on each Swarm node.
- Loki query range returns successful streams for `{collector="alloy",cluster="swarm"}`.
