...
`applications/langgraph/agent/mcp.json` and **`applications/langgraph/subagents/*/mcp.json`** register **mcp-rag** with **`x_api_key_from_env": "MCP_RAG_API_KEY"`** where used. **`framework.agents`** call **`merged_settings()`** before building tools, so set **`MCP_RAG_API_KEY=…`** next to **`OPENAI_API_KEY`** in repo **`.secrets/.env`** when required (see **`.secrets/.env.example`**). Override the base URL with **`HOMELAB_MCP_RAG_URL`** when pointing agents at a different endpoint (for example Compose **mcp-rag-dev**).
