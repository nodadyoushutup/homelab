...
- **NFS:** same homelab repo export as **`rag-engine`** — merged from **`.config/terraform/components/nfs.tfvars`** (see **`terraform/components/nfs.tfvars.example`**). The repo is mounted at **`nfs.target`** inside the container.
