...
- A TFVARS file at `~/.tfvars/node_exporter/app.tfvars` that provides the Docker provider connection (same schema as Dozzle/Jenkins). Example:
...
- The Dozzle and Node Exporter app stacks are structurally similar aside from container specs. Shared locals or helper snippets could reduce duplication if more Swarm services follow this pattern.
- Consider parameterizing the published port/labels via variables if future services require per-environment tweaks without editing Terraform code.
