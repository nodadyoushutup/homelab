...
- **Key paths:** `terraform/module/grafana`, `terraform/module/grafana/config`, `terraform/docker/grafana/{app,config}`, `terraform/docker/grafana/app/pipeline/app.{sh,jenkins}`, `terraform/docker/grafana/config/pipeline/config.{sh,jenkins}`.
...
### App stage (`terraform/docker/grafana/app/pipeline/app.sh`)

```bash
./terraform/docker/grafana/app/pipeline/app.sh \
...
- The stage runs from `terraform/docker/grafana/app`, so the Terraform state only contains Swarm resources—no targeting needed.

### Config stage (`terraform/docker/grafana/config/pipeline/config.sh`)

```bash
./terraform/docker/grafana/config/pipeline/config.sh \
...
- The dedicated `terraform/docker/grafana/config` directory keeps Grafana provider resources in their own state file, so the stage plans/applies normally without manual targeting.

## Jenkins pipelines

- Job names: `grafana/app` and `grafana/config` (under the `grafana` folder).
- Script paths: `terraform/docker/grafana/app/pipeline/app.jenkins` and `terraform/docker/grafana/config/pipeline/config.jenkins`.
...
3. For ad-hoc testing, run `terraform plan` inside `terraform/docker/grafana/config`, but stick to the pipelines for consistency.
...
- Extending the folder:
  ...
  3. If you introduce a brand new dashboard, add it to `default_dashboard_inputs` (or append it to the tfvars `dashboards` list) with the `TrueNAS` folder reference, then re-run `terraform/docker/grafana/config/pipeline/config.sh`.
...
If anything fails, re-run the app stage from `terraform/docker/grafana/app` (or via `terraform/docker/grafana/app/pipeline/app.sh`) to remediate container-level issues before touching config.
