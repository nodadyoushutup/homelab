...
**Query path:** `run_query` in `retrieve/query.py` embeds the user query with `embed_batch` using the configured provider/model/dimensions, then calls Chroma with `query_embeddings`. Retrieval breadth is **engine-driven** via **`RAG_TOP_K`** (default **20** nearest-neighbor chunks). Mismatched providers, models, or dimensions between ingest and query produce unreliable retrieval or Chroma dimension errors; hence the “always go through `rag-engine`” rule.
