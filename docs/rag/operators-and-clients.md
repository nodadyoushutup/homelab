...
| --- | --- | --- |
| ChromaDB | **`terraform/swarm/chromadb/app`** | **`terraform/swarm/chromadb/pipeline/app.sh`** (bespoke; **`swarm.tfvars`** + **`dns.tfvars`** + **`chromadb/app.tfvars`**, not **`swarm_pipeline.sh`**) |
| RAG engine | **`terraform/swarm/rag-engine/app`** | **`terraform/swarm/rag-engine/pipeline/app.sh`** |
| MCP RAG | **`terraform/swarm/mcp-rag/app`** | **`terraform/swarm/mcp-rag/pipeline/app.sh`** |

Tfvars typically live under **`<repo>/.config/terraform/swarm/<stack>/app.tfvars`** (same pattern as other Swarm apps). For **`rag-engine`** and **`mcp-rag`**, set **`env`** (all container settings and secrets) and **`placement`** (`swarm-wk-4`) only; image and ingress ports are fixed in each stack’s **`main.tf`** (repo-wide rules: [swarm-slices.md](../architecture/terraform/swarm-slices.md), placement: [swarm-placement.md](../architecture/terraform/swarm-placement.md)). Bump the image tag in **`main.tf`** after publish, then re-apply.

**Published ports (fixed in Terraform `main.tf` on the Swarm host):** ChromaDB HTTP **8000**, **`rag-engine` → 9015**, **`mcp-rag` → 9016**.

**Images:** publish with **`.github/workflows/docker_build_push.yml`** (`build_target` **`rag-engine`** or **`mcp-rag`**; **`target_registry`** **`github`**, **`zot`**, or **`both`**; **`arm64`** when possible). **`rag-engine`** image tag is pinned in **`terraform/swarm/rag-engine/app/main.tf`** (GHCR). Private registry pulls: set **`registry_auths`** for **`ghcr.io`** and **`zot.nodadyoushutup.com`** under **`swarm_docker_provider_config`** in **`<repo>/.config/terraform/components/swarm.tfvars`** (and **`amd64.tfvars`** / **`arm64.tfvars`** on pool hosts). Zot **`htpasswd`** under **`<repo>/.config/terraform/swarm/zot/`** must use the same username/password as **`registry_auths`** for **`zot.nodadyoushutup.com`**. Config is rendered from **`terraform/swarm/zot/app/files/zot-config.json.tpl`** when the htpasswd file exists at plan time.
