...
STAGE_NAME="MCP GitHub app"
ENTRYPOINT_RELATIVE="terraform/docker/mcp-github/app/pipeline/app.sh"
TERRAFORM_DIR="${ROOT_DIR}/terraform/docker/mcp-github/app"
TFVARS_HOME_DIR="${TFVARS_HOME_DIR:-${TFVARS_DIR:-/mnt/eapp/.tfvars}}"
DEFAULT_TFVARS_FILE="${DEFAULT_TFVARS_FILE:-${TFVARS_HOME_DIR}/mcp-github/app.tfvars}"
...
build_github_image() {
  local image_name="homelab/mcp-github:2026.03.08.4"
  local image_context_dir="${ROOT_DIR}/docker/mcp-github"
  ...
  if [[ "${MCP_GITHUB_REBUILD_IMAGE:-0}" != "1" ]] && DOCKER_HOST="${docker_host}" docker image inspect "${image_name}" >/dev/null 2>&1; then
