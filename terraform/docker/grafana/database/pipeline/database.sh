#!/usr/bin/env bash
# Stage 2 (docs/planning/grafana-plan.md) – database stage deployment
set -euo pipefail
...
STAGE_NAME="Grafana database"
ENTRYPOINT_RELATIVE="terraform/docker/grafana/database/pipeline/database.sh"
TERRAFORM_DIR="${ROOT_DIR}/terraform/docker/grafana/database"

TFVARS_HOME_DIR="${TFVARS_HOME_DIR:-${TFVARS_DIR:-/mnt/eapp/.tfvars}}"
DEFAULT_TFVARS_FILE="${DEFAULT_TFVARS_FILE:-${TFVARS_HOME_DIR}/grafana/database.tfvars}"
...
ensure_app_state_exists() {
  local app_tf_dir="${ROOT_DIR}/terraform/docker/grafana/app"
