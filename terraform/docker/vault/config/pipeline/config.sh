...
ENTRYPOINT_RELATIVE="terraform/docker/vault/config/pipeline/config.sh"
TERRAFORM_DIR="${ROOT_DIR}/terraform/docker/vault/config"
...
pipeline_pre_terraform() {
  [[ -f "${VAULT_INIT_FILE}" ]] || {
    echo "[ERR] Missing ${VAULT_INIT_FILE}. Run terraform/docker/vault/app/pipeline/app.sh first to bootstrap Vault." >&2
    exit 1
  }
