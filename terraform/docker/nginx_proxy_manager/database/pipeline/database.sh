#!/usr/bin/env bash
# Stage 2 (docs/planning/nginx-proxy-manager-plan.md) – database stage deployment
set -euo pipefail
...
ENTRYPOINT_RELATIVE="terraform/docker/nginx_proxy_manager/database/pipeline/database.sh"
TERRAFORM_DIR="${ROOT_DIR}/terraform/docker/nginx_proxy_manager/database"
...
ensure_app_state_exists() {
  local app_tf_dir="${ROOT_DIR}/terraform/docker/nginx_proxy_manager/app"
