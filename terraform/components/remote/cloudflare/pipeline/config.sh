#!/usr/bin/env bash
# Bespoke Cloudflare config deploy (intentional during the AGENTS.md audit campaign).
# Self-contained entrypoint — do not migrate onto shared *_pipeline.sh mid-audit.
# Slice tfvars plus the Cloudflare provider credentials (config-id
# terraform/providers/cloudflare); no other shared provider/DNS/NFS var-files.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../../../../.." && pwd)"
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/remote/cloudflare/config"
...
# shellcheck source=../../../scripts/terraform/cloudflare_tfvars_env.sh
source "${ROOT_DIR}/scripts/terraform/cloudflare_tfvars_env.sh"
...
SLICE_TFVARS="${CLOUDFLARE_CONFIG_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${CLOUDFLARE_CONFIG_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/remote/cloudflare/pipeline/config.sh [options] [slice_tfvars] [backend_config]

Deploy Cloudflare config (terraform init, plan, apply).
...
Environment overrides: CLOUDFLARE_CONFIG_TFVARS, CLOUDFLARE_CONFIG_BACKEND, CONFIG_DIR (default: <repo>/.config)
USAGE
}
...
require_file "cloudflare credentials tfvars" "${CLOUDFLARE_TFVARS}"

echo "Terraform dir:     ${TERRAFORM_DIR}"
echo "Slice tfvars:      ${SLICE_TFVARS}"
echo "Cloudflare creds:  ${CLOUDFLARE_TFVARS}"
echo "Backend config:    ${BACKEND_CONFIG}"

cd "${TERRAFORM_DIR}"


echo "[STEP] terraform init (Cloudflare config)"
