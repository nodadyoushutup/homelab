#!/usr/bin/env bash
# Bespoke FortiGate config deploy (intentional during the AGENTS.md audit campaign).
# Self-contained entrypoint — do not migrate onto shared *_pipeline.sh mid-audit.
# Slice tfvars plus the FortiGate provider credentials (config-id
# terraform/providers/fortigate); no other shared provider/DNS/NFS var-files.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../../../../.." && pwd)"
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/network/fortigate/config"
...
# shellcheck source=../../../scripts/terraform/fortigate_tfvars_env.sh
source "${ROOT_DIR}/scripts/terraform/fortigate_tfvars_env.sh"
...
SLICE_TFVARS="${FORTIGATE_CONFIG_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${FORTIGATE_CONFIG_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/network/fortigate/pipeline/config.sh [options] [slice_tfvars] [backend_config]

Deploy FortiGate config (terraform init, plan, apply).
...
Environment overrides: FORTIGATE_CONFIG_TFVARS, FORTIGATE_CONFIG_BACKEND, CONFIG_DIR (default: <repo>/.config)
USAGE
}
...
require_file "fortigate credentials tfvars" "${FORTIGATE_TFVARS}"

echo "Terraform dir:     ${TERRAFORM_DIR}"
echo "Slice tfvars:      ${SLICE_TFVARS}"
echo "FortiGate creds:   ${FORTIGATE_TFVARS}"
echo "Backend config:    ${BACKEND_CONFIG}"

cd "${TERRAFORM_DIR}"


echo "[STEP] terraform init (FortiGate config)"
