...
if [[ $# -gt 0 ]]; then
  ...
  echo "      expected tfvars:  <TFVARS_HOME>/terraform/components/swarm/vault/config.tfvars" >&2
...
VAULT_TFVARS_DIR="${VAULT_TFVARS_HOME}/terraform/components/swarm/vault"
...
ENTRYPOINT_RELATIVE="terraform/components/swarm/vault/pipeline/config.sh"
...
pipeline_pre_terraform() {
  ...
  if ! "${_merge_py[@]}" "${ROOT_DIR}/scripts/terraform/vault_merge_config_secrets.py" \
    ...
    --vault-config-tfvars "${TFVARS_HOME_DIR}/terraform/components/swarm/vault/config.tfvars" \
