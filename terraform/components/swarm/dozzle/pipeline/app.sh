#!/usr/bin/env bash
# Bespoke Dozzle Swarm deploy (intentional during the AGENTS.md audit campaign).
...
SLICE_TFVARS="${DOZZLE_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${DOZZLE_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/dozzle/pipeline/app.sh [options] [slice_tfvars] [backend_config]
...
Environment overrides: DOZZLE_APP_TFVARS, DOZZLE_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t dozzle-app-terraform-init-XXXXXX)"
