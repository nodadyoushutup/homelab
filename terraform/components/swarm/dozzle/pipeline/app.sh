#!/usr/bin/env bash
# Bespoke Dozzle Swarm deploy — no shared swarm_pipeline / scripts/terraform helpers.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/dozzle/app"
...
APP_TFVARS="${DOZZLE_APP_TFVARS:-${DEFAULT_APP_TFVARS}}"
BACKEND_CONFIG="${DOZZLE_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/dozzle/pipeline/app.sh [options] [app_tfvars] [backend_config]

Deploy Dozzle on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: SWARM_DOCKER_PROVIDER_TFVARS, SWARM_DNS_PROVIDER_TFVARS, DOZZLE_APP_TFVARS, DOZZLE_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t dozzle-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (Dozzle)"
