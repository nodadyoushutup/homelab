#!/usr/bin/env bash
# Bespoke Cloud image repository Swarm deploy (intentional during the AGENTS.md audit campaign).
...
SLICE_TFVARS="${CLOUD_IMAGE_REPOSITORY_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${CLOUD_IMAGE_REPOSITORY_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/cloud-image-repository/pipeline/app.sh [options] [slice_tfvars] [backend_config]
...
Environment overrides: CLOUD_IMAGE_REPOSITORY_APP_TFVARS, CLOUD_IMAGE_REPOSITORY_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t cloud-image-repository-app-terraform-init-XXXXXX)"
