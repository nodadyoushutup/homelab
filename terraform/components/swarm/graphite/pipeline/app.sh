#!/usr/bin/env bash
# Bespoke Graphite Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/graphite/app"
...
SLICE_TFVARS="${GRAPHITE_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${GRAPHITE_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/graphite/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy Graphite on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: GRAPHITE_APP_TFVARS, GRAPHITE_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t graphite-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (Graphite)"
