#!/usr/bin/env bash
# Bespoke qBittorrent exporter Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/qbittorrent-exporter/app"
...
SLICE_TFVARS="${QBITTORRENT_EXPORTER_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${QBITTORRENT_EXPORTER_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/qbittorrent-exporter/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy qBittorrent exporter on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: QBITTORRENT_EXPORTER_APP_TFVARS, QBITTORRENT_EXPORTER_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t qbittorrent-exporter-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (qBittorrent exporter)"
