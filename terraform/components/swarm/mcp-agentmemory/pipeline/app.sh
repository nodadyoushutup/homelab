#!/usr/bin/env bash
# Bespoke MCP agentmemory Swarm deploy (intentional during the AGENTS.md audit campaign).
...
SLICE_TFVARS="${MCP_AGENTMEMORY_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${MCP_AGENTMEMORY_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/mcp-agentmemory/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy MCP agentmemory on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: MCP_AGENTMEMORY_APP_TFVARS, MCP_AGENTMEMORY_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t mcp-agentmemory-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (MCP agentmemory)"
