#!/usr/bin/env bash
# Bespoke Node exporter Swarm deploy (intentional during the AGENTS.md audit campaign).
...
SLICE_TFVARS="${NODE_EXPORTER_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${NODE_EXPORTER_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/node_exporter/pipeline/app.sh [options] [slice_tfvars] [backend_config]
...
Environment overrides: NODE_EXPORTER_APP_TFVARS, NODE_EXPORTER_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t node_exporter-app-terraform-init-XXXXXX)"
