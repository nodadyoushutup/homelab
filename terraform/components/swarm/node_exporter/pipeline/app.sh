#!/usr/bin/env bash
# Bespoke Node Exporter Swarm deploy — no shared swarm_pipeline / scripts/terraform helpers.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/node_exporter/app"
...
DOCKER_TFVARS="${SWARM_DOCKER_PROVIDER_TFVARS:-${NODE_EXPORTER_DOCKER_TFVARS:-${DEFAULT_DOCKER_TFVARS}}}"
DNS_TFVARS="${SWARM_DNS_PROVIDER_TFVARS:-${NODE_EXPORTER_DNS_TFVARS:-${DEFAULT_DNS_TFVARS}}}"
APP_TFVARS="${NODE_EXPORTER_APP_TFVARS:-${DEFAULT_APP_TFVARS}}"
BACKEND_CONFIG="${NODE_EXPORTER_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/node_exporter/pipeline/app.sh [options] [app_tfvars] [backend_config]

Deploy Node Exporter on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: SWARM_DOCKER_PROVIDER_TFVARS, SWARM_DNS_PROVIDER_TFVARS, NODE_EXPORTER_APP_TFVARS, NODE_EXPORTER_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t node-exporter-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (Node Exporter)"
