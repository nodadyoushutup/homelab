#!/usr/bin/env bash
# Bespoke Prometheus Swarm deploy (intentional during the AGENTS.md audit campaign).
...
SLICE_TFVARS="${PROMETHEUS_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${PROMETHEUS_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/prometheus/pipeline/app.sh [options] [slice_tfvars] [backend_config]
...
Environment overrides: PROMETHEUS_APP_TFVARS, PROMETHEUS_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
echo "[STEP] terraform init (Prometheus)"
