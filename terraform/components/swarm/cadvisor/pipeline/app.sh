...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/cadvisor/app"
...
usage() {
  cat <<USAGE
Usage: terraform/components/swarm/cadvisor/pipeline/app.sh [options] [app_tfvars] [backend_config]
...
Environment overrides: CADVISOR_APP_TFVARS, CADVISOR_BACKEND, CONFIG_DIR (default: <repo>/.config)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t cadvisor-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (cAdvisor)"
