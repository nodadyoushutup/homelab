#!/usr/bin/env bash
# Bespoke Graylog app Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/graylog/app"
...
SLICE_TFVARS="${GRAYLOG_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${GRAYLOG_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/graylog/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy Graylog app on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: GRAYLOG_APP_TFVARS, GRAYLOG_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t graylog-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (Graylog app)"
