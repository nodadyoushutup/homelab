#!/usr/bin/env bash
# Bespoke Graylog database Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/graylog/database"
...
SLICE_TFVARS="${GRAYLOG_DATABASE_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${GRAYLOG_DATABASE_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/graylog/pipeline/database.sh [options] [slice_tfvars] [backend_config]

Deploy Graylog database on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: GRAYLOG_DATABASE_TFVARS, GRAYLOG_DATABASE_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
