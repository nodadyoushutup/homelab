#!/usr/bin/env bash
# Bespoke Zot Swarm deploy (intentional during the AGENTS.md audit campaign).
...
SLICE_TFVARS="${ZOT_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${ZOT_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/zot/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy Zot on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: ZOT_APP_TFVARS, ZOT_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t zot-app-terraform-init-XXXXXX)"
