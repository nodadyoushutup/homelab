#!/usr/bin/env bash
# Bespoke Grafana app Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/grafana/app"
...
SLICE_TFVARS="${GRAFANA_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${GRAFANA_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/grafana/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy Grafana app on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: GRAFANA_APP_TFVARS, GRAFANA_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t grafana-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (Grafana app)"
