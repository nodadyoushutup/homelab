#!/usr/bin/env bash
# Bespoke ChromaDB Swarm deploy — no shared swarm_pipeline / scripts/terraform helpers.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/chromadb/app"
...
usage() {
  cat <<USAGE
Usage: terraform/components/swarm/chromadb/pipeline/app.sh [options] [app_tfvars] [backend_config]

Deploy ChromaDB on Docker Swarm (terraform init, plan, apply).
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t chromadb-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (ChromaDB)"
