#!/usr/bin/env bash
# Bespoke Jenkins agent amd64 deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/jenkins-agent-amd64/app"
...
SLICE_TFVARS="${JENKINS_AGENT_AMD64_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${JENKINS_AGENT_AMD64_APP_BACKEND:-${DEFAULT_BACKEND}}"
...
usage() {
  cat <<USAGE
Usage: terraform/components/swarm/jenkins-agent-amd64/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy Jenkins agent amd64 (terraform init, plan, apply).
...
Environment overrides: JENKINS_AGENT_AMD64_APP_TFVARS, JENKINS_AGENT_AMD64_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
