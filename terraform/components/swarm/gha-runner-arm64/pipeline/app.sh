#!/usr/bin/env bash
# Bespoke GitHub Actions ARM64 runner deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/gha-runner-arm64/app"
...
SLICE_TFVARS="${GHA_RUNNER_ARM64_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${GHA_RUNNER_ARM64_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/gha-runner-arm64/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy GitHub Actions ARM64 runner (terraform init, plan, apply).
...
Environment overrides: GHA_RUNNER_ARM64_APP_TFVARS, GHA_RUNNER_ARM64_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t gha-runner-arm64-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (GitHub Actions ARM64 runner)"
...
echo "[STEP] terraform plan (GitHub Actions ARM64 runner)"
if ! terraform plan "${PLAN_ARGS[@]}"; then
  echo "[ERR] terraform plan failed" >&2
  exit 1
fi

echo "[STEP] terraform apply (GitHub Actions ARM64 runner)"
if ! terraform apply -input=false -auto-approve "${PLAN_ARGS[@]}"; then
  echo "[ERR] terraform apply failed" >&2
  exit 1
fi

echo "[DONE] GitHub Actions ARM64 runner apply complete."
