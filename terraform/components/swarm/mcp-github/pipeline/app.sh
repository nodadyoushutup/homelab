#!/usr/bin/env bash
# Bespoke MCP GitHub Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/mcp-github/app"
...
SLICE_TFVARS="${MCP_GITHUB_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${MCP_GITHUB_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/mcp-github/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy MCP GitHub on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: MCP_GITHUB_APP_TFVARS, MCP_GITHUB_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t mcp-github-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (MCP GitHub)"
