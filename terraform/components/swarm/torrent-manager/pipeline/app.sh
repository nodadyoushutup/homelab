#!/usr/bin/env bash
# Bespoke torrent-manager Swarm deploy.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/torrent-manager/app"
...
DOCKER_TFVARS="${SWARM_DOCKER_PROVIDER_TFVARS:-${TORRENT_MANAGER_DOCKER_TFVARS:-${DEFAULT_DOCKER_TFVARS}}}"
DNS_TFVARS="${SWARM_DNS_PROVIDER_TFVARS:-${TORRENT_MANAGER_DNS_TFVARS:-${DEFAULT_DNS_TFVARS}}}"
APP_TFVARS="${TORRENT_MANAGER_APP_TFVARS:-${DEFAULT_APP_TFVARS}}"
BACKEND_CONFIG="${TORRENT_MANAGER_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/torrent-manager/pipeline/app.sh [options] [app_tfvars] [backend_config]

Deploy torrent-manager on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: SWARM_DOCKER_PROVIDER_TFVARS, SWARM_DNS_PROVIDER_TFVARS,
TORRENT_MANAGER_APP_TFVARS, TORRENT_MANAGER_BACKEND, CONFIG_DIR
USAGE
}
...
require_file "torrent-manager config yaml" "${CONFIG_DIR}/torrent-manager/config.yaml"
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t torrent-manager-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (torrent-manager)"
