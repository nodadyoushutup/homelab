#!/usr/bin/env bash
# Bespoke Nginx Proxy Manager app Swarm deploy (intentional during the AGENTS.md audit campaign).
...
SLICE_TFVARS="${NGINX_PROXY_MANAGER_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${NGINX_PROXY_MANAGER_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/nginx_proxy_manager/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy Nginx Proxy Manager app on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: NGINX_PROXY_MANAGER_APP_TFVARS, NGINX_PROXY_MANAGER_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t nginx_proxy_manager-app-terraform-init-XXXXXX)"
