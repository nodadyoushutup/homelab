#!/usr/bin/env bash
# Bespoke Nginx Proxy Manager app Swarm deploy — no shared swarm_pipeline.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/nginx_proxy_manager/app"
...
DOCKER_TFVARS="${SWARM_DOCKER_PROVIDER_TFVARS:-${NPM_APP_DOCKER_TFVARS:-${DEFAULT_DOCKER_TFVARS}}}"
DNS_TFVARS="${SWARM_DNS_PROVIDER_TFVARS:-${NPM_APP_DNS_TFVARS:-${DEFAULT_DNS_TFVARS}}}"
APP_TFVARS="${NPM_APP_TFVARS:-${DEFAULT_APP_TFVARS}}"
BACKEND_CONFIG="${NPM_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/nginx_proxy_manager/pipeline/app.sh [options] [app_tfvars] [backend_config]

Deploy Nginx Proxy Manager on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: SWARM_DOCKER_PROVIDER_TFVARS, SWARM_DNS_PROVIDER_TFVARS, NPM_APP_TFVARS, NPM_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t npm-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (Nginx Proxy Manager app)"
