#!/usr/bin/env bash
# Bespoke Nginx Proxy Manager database Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/nginx_proxy_manager/database"
...
SLICE_TFVARS="${NGINX_PROXY_MANAGER_DATABASE_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${NGINX_PROXY_MANAGER_DATABASE_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/nginx_proxy_manager/pipeline/database.sh [options] [slice_tfvars] [backend_config]

Deploy Nginx Proxy Manager database on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: NGINX_PROXY_MANAGER_DATABASE_TFVARS, NGINX_PROXY_MANAGER_DATABASE_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t nginx_proxy_manager-database-terraform-init-XXXXXX)"
