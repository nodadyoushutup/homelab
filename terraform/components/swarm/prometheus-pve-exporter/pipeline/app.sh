#!/usr/bin/env bash
# Bespoke Prometheus PVE exporter Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/prometheus-pve-exporter/app"
...
SLICE_TFVARS="${PROMETHEUS_PVE_EXPORTER_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${PROMETHEUS_PVE_EXPORTER_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/prometheus-pve-exporter/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy Prometheus PVE exporter on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: PROMETHEUS_PVE_EXPORTER_APP_TFVARS, PROMETHEUS_PVE_EXPORTER_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t prometheus-pve-exporter-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (Prometheus PVE exporter)"
