#!/usr/bin/env bash
# Bespoke Container housekeeping Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/container-housekeeping/app"
...
SLICE_TFVARS="${CONTAINER_HOUSEKEEPING_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${CONTAINER_HOUSEKEEPING_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/container-housekeeping/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy Container housekeeping on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: CONTAINER_HOUSEKEEPING_APP_TFVARS, CONTAINER_HOUSEKEEPING_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t container-housekeeping-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (Container housekeeping)"
