#!/usr/bin/env bash
# Bespoke MCP FortiGate Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/mcp-fortigate/app"
...
SLICE_TFVARS="${MCP_FORTIGATE_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${MCP_FORTIGATE_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/mcp-fortigate/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy MCP FortiGate on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: MCP_FORTIGATE_APP_TFVARS, MCP_FORTIGATE_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t mcp-fortigate-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (MCP FortiGate)"
