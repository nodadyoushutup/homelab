#!/usr/bin/env bash
# Bespoke VictoriaMetrics Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/victoriametrics/app"
...
SLICE_TFVARS="${VICTORIAMETRICS_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${VICTORIAMETRICS_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/victoriametrics/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy VictoriaMetrics on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: VICTORIAMETRICS_APP_TFVARS, VICTORIAMETRICS_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t victoriametrics-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (VictoriaMetrics)"
