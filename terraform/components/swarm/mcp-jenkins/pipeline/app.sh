#!/usr/bin/env bash
# Bespoke MCP Jenkins Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/mcp-jenkins/app"
...
SLICE_TFVARS="${MCP_JENKINS_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${MCP_JENKINS_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/mcp-jenkins/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy MCP Jenkins on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: MCP_JENKINS_APP_TFVARS, MCP_JENKINS_APP_BACKEND, CONFIG_DIR (default: <repo>/.config)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t mcp-jenkins-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (MCP Jenkins)"
