...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/jenkins-controller/config"
...
SLICE_TFVARS="${JENKINS_CONTROLLER_CONFIG_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${JENKINS_CONTROLLER_CONFIG_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/jenkins-controller/pipeline/config.sh [options] [slice_tfvars] [backend_config]

Deploy Jenkins controller config on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: JENKINS_CONTROLLER_CONFIG_TFVARS, JENKINS_CONTROLLER_CONFIG_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
echo "[STEP] terraform apply (Jenkins controller config)"
if ! terraform apply -input=false -auto-approve "${PLAN_ARGS[@]}"; then
  echo "[ERR] terraform apply failed" >&2
  exit 1
fi

echo "[DONE] Jenkins controller config apply complete."
