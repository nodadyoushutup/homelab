#!/usr/bin/env bash
# Bespoke Jenkins controller app Swarm deploy (intentional during the AGENTS.md audit campaign).
...
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/swarm/jenkins-controller/app"
...
SLICE_TFVARS="${JENKINS_CONTROLLER_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${JENKINS_CONTROLLER_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/swarm/jenkins-controller/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy Jenkins controller app on Docker Swarm (terraform init, plan, apply).
...
Environment overrides: JENKINS_CONTROLLER_APP_TFVARS, JENKINS_CONTROLLER_APP_BACKEND, CONFIG_DIR (from .config/docker/site.env)
USAGE
}
...
run_terraform_init() {
  local init_log
  init_log="$(mktemp -t jenkins-controller-app-terraform-init-XXXXXX)"
...
echo "[STEP] terraform init (Jenkins controller app)"
