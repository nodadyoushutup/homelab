...
ENTRYPOINT_RELATIVE="terraform/components/runners/jenkins-agent-arm64/pipeline/app.sh"
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/runners/jenkins-agent-arm64/app"

JENKINS_AGENT_ARM64_TFVARS_DIR="${JENKINS_AGENT_ARM64_TFVARS_DIR:-${CONFIG_DIR:-${ROOT_DIR}/.config}/terraform/components/runners/jenkins-agent-arm64}"
...
SWARM_DOCKER_PROVIDER_TFVARS="${SWARM_DOCKER_PROVIDER_TFVARS:-$(homelab_resolve_config_path "${TFVARS_HOME_DIR}" "terraform/components/runners/arm64")}"
...
if [[ ! -f "${SWARM_DOCKER_PROVIDER_TFVARS}" ]]; then
  ...
  echo "[ERR] Create it from terraform/components/runners/arm64.tfvars.example." >&2
  exit 1
fi
