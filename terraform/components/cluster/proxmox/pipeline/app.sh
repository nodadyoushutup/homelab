#!/usr/bin/env bash
# Bespoke Proxmox cluster deploy (intentional during the AGENTS.md audit campaign).
# Self-contained entrypoint — do not migrate onto shared *_pipeline.sh mid-audit.
# Slice tfvars plus the Proxmox provider credentials (config-id
# terraform/providers/proxmox); no other shared provider/DNS/NFS var-files.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../../../../.." && pwd)"
TERRAFORM_DIR="${ROOT_DIR}/terraform/components/cluster/proxmox/app"
...
# shellcheck source=../../../scripts/terraform/proxmox_tfvars_env.sh
source "${ROOT_DIR}/scripts/terraform/proxmox_tfvars_env.sh"
...
SLICE_TFVARS="${PROXMOX_APP_TFVARS:-${DEFAULT_SLICE_TFVARS}}"
BACKEND_CONFIG="${PROXMOX_APP_BACKEND:-${DEFAULT_BACKEND}}"

usage() {
  cat <<USAGE
Usage: terraform/components/cluster/proxmox/pipeline/app.sh [options] [slice_tfvars] [backend_config]

Deploy Proxmox cluster (terraform init, plan, apply).
...
Environment overrides: PROXMOX_APP_TFVARS, PROXMOX_APP_BACKEND, CONFIG_DIR (default: <repo>/.config)
USAGE
}
...
require_file "proxmox credentials tfvars" "${PROXMOX_TFVARS}"

echo "Terraform dir:     ${TERRAFORM_DIR}"
echo "Slice tfvars:      ${SLICE_TFVARS}"
echo "Proxmox creds:     ${PROXMOX_TFVARS}"
echo "Backend config:    ${BACKEND_CONFIG}"

cd "${TERRAFORM_DIR}"


echo "[STEP] terraform init (Proxmox cluster)"
