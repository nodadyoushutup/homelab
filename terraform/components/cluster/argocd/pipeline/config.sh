...
# Slice tfvars plus the Argo CD provider credentials (config-id
# terraform/providers/argocd); no other shared provider/DNS/NFS var-files.
set -euo pipefail
...
# shellcheck source=../../../scripts/terraform/argocd_tfvars_env.sh
source "${ROOT_DIR}/scripts/terraform/argocd_tfvars_env.sh"
...
Environment overrides: ARGOCD_CONFIG_TFVARS, ARGOCD_CONFIG_BACKEND, CONFIG_DIR (default: <repo>/.config)
USAGE
}
...
require_file "argocd credentials tfvars" "${ARGOCD_TFVARS}"

echo "Terraform dir:     ${TERRAFORM_DIR}"
echo "Slice tfvars:      ${SLICE_TFVARS}"
echo "Argo CD creds:     ${ARGOCD_TFVARS}"
