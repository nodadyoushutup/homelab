...
variable "image_reference" {
  description = "Cloudflare MCP image to run."
  type        = string
  default     = "homelab/mcp-cloudflare:0.0.1"
}

variable "env_file_path" {
  description = "Optional dotenv file containing Cloudflare MCP secrets and settings."
  type        = string
  default     = ""
}
...
variable "published_port" {
  description = "Swarm ingress port exposed for the Cloudflare MCP HTTP endpoint."
  type        = number
  default     = 18204
}
