...
variable "image_reference" {
  description = "ast-grep MCP image to run."
  type        = string
  default     = "homelab/mcp-ast-grep:2026.04.16.2"
}
...
variable "published_port" {
  description = "Swarm ingress port exposed for the ast-grep MCP HTTP endpoint."
  type        = number
  default     = 18202
}
...
variable "nfs_code_device" {
  description = "NFS export for repo code (mounted read-only at /mnt/eapp/code in the task)."
  type        = string
