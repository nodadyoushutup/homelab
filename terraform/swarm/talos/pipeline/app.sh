...
ENTRYPOINT_RELATIVE="terraform/swarm/talos/pipeline/app.sh"
TERRAFORM_DIR="${ROOT_DIR}/terraform/swarm/talos/app"
...
MANAGED_TALOSCONFIG_OUTPUT_PATH="${MANAGED_TALOSCONFIG_OUTPUT_PATH:-${TFVARS_HOME_DIR}/terraform/swarm/talos/app/talosconfig}"
MANAGED_KUBECONFIG_OUTPUT_PATH="${MANAGED_KUBECONFIG_OUTPUT_PATH:-${TFVARS_HOME_DIR}/terraform/swarm/talos/app/kubeconfig}"
...
redirect_local_file_outputs_if_unwritable() {
  ...
  if [[ "${OVERRIDE_TALOSCONFIG_OUTPUT_PATH}" == "${MANAGED_TALOSCONFIG_OUTPUT_PATH}" && "${OVERRIDE_KUBECONFIG_OUTPUT_PATH}" == "${MANAGED_KUBECONFIG_OUTPUT_PATH}" ]]; then
    echo "[INFO] Redirecting Talos local file outputs to shared managed paths under ${TFVARS_HOME_DIR}/terraform/swarm/talos/app"
