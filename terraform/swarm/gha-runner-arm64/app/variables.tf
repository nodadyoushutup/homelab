...
variable "replicas" {
  description = "Number of runner containers on the pool host (see swarm_docker_provider_config.docker in docker_arm64_pool.tfvars)."
  type        = number
  default     = 2
}
...
variable "swarm_docker_provider_config" {
  description = <<-EOT
    ...
    This ARM64 runner pipeline also merges docker_arm64_pool.tfvars last; that file sets
    `swarm_docker_provider_config` for the pool host (standalone `docker_container`, not Swarm scheduling).
    Include `docker` and `registry_auths` in the pool tfvars.
  EOT
  type        = any
  default     = {}
}
