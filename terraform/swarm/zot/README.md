...
**`terraform/swarm/zot/app/files/zot-config.json.tpl`** — standard Zot config; Terraform adds `http.auth` / `accessControl` only if `fileexists(htpasswd_path)`.

Usernames in **`htpasswd`** must match `registry_auths` for `zot.nodadyoushutup.com` in **`.config/terraform/components/{swarm,amd64,arm64}.tfvars`**. Access control uses **`defaultPolicy`** (any authenticated user).
...
```bash
terraform/swarm/zot/pipeline/app.sh apply
