...
ENTRYPOINT_RELATIVE="pipelines/terraform/runners/jenkins-agent-amd64/app.sh"
TERRAFORM_DIR="${ROOT_DIR}/terraform/runners/jenkins-agent-amd64/app"

JENKINS_AGENT_AMD64_TFVARS_DIR="${JENKINS_AGENT_AMD64_TFVARS_DIR:-${CONFIG_DIR:-${ROOT_DIR}/.config}/terraform/runners/jenkins-agent-amd64}"
...
if [[ ! -f "${SWARM_DOCKER_PROVIDER_TFVARS}" ]]; then
  ...
  echo "[ERR] Create it from homelab terraform/components/amd64.tfvars.example." >&2
  exit 1
fi
