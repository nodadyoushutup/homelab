...
13. For pure workflow transitions, send only required transition arguments unless Jira reports required transition fields. Some transition-comment inputs expect Atlassian Document Format, so do not send plain-text comments through transition calls.
14. If a transition also needs a human-visible note, transition first and then add a separate Jira comment.
15. Ask follow-up questions only for real stage blockers, and tie each question to the missing information needed to complete or exit the current stage.
16. When the requested work completes the current stage, explicitly say so and invite the caller to move to the most likely next stage from the workflow map.
17. If several next transitions are valid, recommend the most likely next step and mention the other valid branch only when it materially affects the decision.
18. If the user does not specify a project or board for a new issue, use `Homelab` with key `HOME` as the runtime's default and fallback unless Jira blocks it. Only use a different project or board when the user explicitly requests one or live Jira metadata proves the work belongs somewhere else.
