...
pool from `terraform/swarm/gha-runner-amd64/app`. Point each stack at its pool host with
`provider_config.docker.host` (and matching `ssh_opts`) in the stack’s `app.tfvars`; the
shared `/mnt/eapp/config/providers/docker.tfvars` still supplies registry credentials.
...
- **`github_runner_replicas`** is the number of parallel runner containers on **that pool host** (override in tfvars). Apply both stacks so you get the intended AMD64 and ARM64 capacity.
...
**ARM64 pool host choice:** point `provider_config.docker.host` at an AArch64 machine that actually exposes `/dev/kvm` if you expect Packer with `-accel kvm`. Small SBCs often omit KVM; pick another ARM host in tfvars if needed.
...
2. Bump `github_runner_image` / registry reference if it pins an old tag.
