...
repo, the ARM64 pool is managed from `terraform/components/swarm/gha-runner-arm64/app` and the AMD64
pool from `terraform/components/swarm/gha-runner-amd64/app`. Pool Docker SSH targets and
registry auth live in each pool’s self-contained slice tfvars
(`.config/terraform/components/swarm/gha-runner-{amd64,arm64}/app.tfvars`).
...
- Default runner labels include **`build,kvm`** on both pools (see each `variables.tf`). Workflows that need hardware acceleration (for example **Packer** in `.github/workflows/packer.yml`) should use `runs-on` labels that include **`kvm`** so jobs land on these pools only after you confirm the **pool host** is KVM-capable.
...
2. Bump the literal `image` in `terraform/components/swarm/gha-runner-amd64/app/main.tf` if you need a new tag; same for ARM64.
