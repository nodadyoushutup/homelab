"""In-memory working copy of the VictoriaMetrics app settings, backed by app.tfvars.
...
class VictoriaMetricsConfigStore:
    """Thread-safe working copy of the VictoriaMetrics app settings."""

    def __init__(self, path: Path = VICTORIAMETRICS_APP_TFVARS) -> None:
        self._path = path
    ...
    def render(self) -> str:
        """Return the rendered tfvars for the working copy."""
        with self._lock:
    ...
    def status(self) -> dict:
        ...
        with self._lock:
            disk = read_victoriametrics_tfvars(self._path) or default_settings()
    ...
    def write(self) -> Path:
        ...
        with self._lock:
            path = write_victoriametrics_tfvars(self._working, self._path)
    ...
    def reload(self) -> None:
        ...
        with self._lock:
            settings = read_victoriametrics_tfvars(self._path) or default_settings()
            self._working = copy.deepcopy(settings)
            self._baseline = copy.deepcopy(settings)


__all__ = ["VictoriaMetricsConfigStore", "ConfigValidationError"]
