...
This directory vendors the upstream `langchain-ai/agent-chat-ui` application as
the repo-owned source for the homelab LangChain Agent Chat deployment.
...
You can control the visibility of messages within LangChain Agent Chat in two main ways:
...
LangChain Agent Chat supports rendering artifacts in the chat. Artifacts are rendered in a side panel to the right of the chat. To render an artifact, you can obtain the artifact context from the `thread.meta.artifact` field. Here's a sample utility hook for obtaining the artifact context:
...
Once you've set it up on your deployment, you should make the following changes to LangChain Agent Chat:
