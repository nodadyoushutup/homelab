...
#
# Default loads <repo>/.config/docker/rag.env (via load_root_env.sh) for RAG_ENGINE_*.
...
Default: POST \$RAG_ENGINE_BASE_URL/v1/backfill (see .config/docker/rag.env). Same flags as
...
load_repo_env() {
  ...
  source "${REPO_ROOT}/scripts/terraform/load_root_env.sh"
}
