...
usage() {
  cat <<'EOF_USAGE'
Usage: scripts/docker/build_push.sh --version <version> --target_registry <github|zot|both> --build_target <target> [options]
...
registry_login_zot() {
  ...
  username="${ZOT_USERNAME_VALUE:-${ZOT_REGISTRY_USERNAME:-}}"
  password="${ZOT_PASSWORD_VALUE:-${ZOT_REGISTRY_PASSWORD:-}}"

  [[ -n "${username}" ]] || die "Zot username is required via --zot_username or ZOT_REGISTRY_USERNAME"
  [[ -n "${password}" ]] || die "Zot password is required via --zot_password or ZOT_REGISTRY_PASSWORD"

  printf '%s' "${password}" | docker login "${ZOT_REGISTRY}" --username "${username}" --password-stdin >/dev/null
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --github_token)
      ...
      ;;
    --zot_username)
      ZOT_USERNAME_VALUE="$2"
      shift 2
      ;;
    --zot_password)
      ZOT_PASSWORD_VALUE="$2"
      shift 2
      ;;
    -h|--help)
      usage
...
case "${PHASE}" in
  all|build-direct-arch|publish-direct-manifest)
    ;;
  *)
