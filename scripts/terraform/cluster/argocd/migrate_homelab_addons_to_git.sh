...
TF_DIR="${ROOT_DIR}/terraform/components/cluster/argocd/config"

# Prefer repo .config via load_root_env; unset any inherited CONFIG_DIR override before lookup.
...
if ! terraform state list 2>/dev/null | grep -q 'argocd_application.argocd_management'; then
  ...
  terraform import \
    -var-file="${CONFIG_DIR}/terraform/components/cluster/argocd/config.tfvars" \
