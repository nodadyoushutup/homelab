...
if [[ -z "${SCRIPT_DIR_FOR_STAGE}" || -z "${ROOT_DIR}" || -z "${PIPELINE_SCRIPT_ROOT}" ]]; then
  echo "[ERR] Stage script must define SCRIPT_DIR, ROOT_DIR, and PIPELINE_SCRIPT_ROOT before sourcing cluster_pipeline.sh" >&2
  exit 1
fi

if [[ -z "${SERVICE_NAME}" || -z "${STAGE_NAME}" ]]; then
  echo "[ERR] Stage script must define SERVICE_NAME and STAGE_NAME before sourcing cluster_pipeline.sh" >&2
  exit 1
fi
...
TERRAFORM_DIR="${TERRAFORM_DIR:-${ROOT_DIR}/terraform/components/cluster/${SERVICE_NAME}}"
