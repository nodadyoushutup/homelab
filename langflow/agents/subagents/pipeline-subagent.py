...
class PipelineAgentComponent(ToolCallingAgentComponent):
    display_name: str = "Pipeline Agent"
    description: str = (
        "Inspect repo-managed stage pipelines and run bounded deployment actions with tools, "
        "then return source-backed analysis."
    )
    ...
    inputs = [
        ...
        MultilineInput(
            ...
            value=(
                "You are the Pipeline agent. Use tools to inspect repo-managed stage pipeline entrypoints, "
                "shared wrappers, tfvars paths, and execution constraints, and to run bounded pipeline actions "
                "when the request is actionable. Prioritize confirmed, source-backed findings. Separate facts "
                "from assumptions and risks, stay within the provided pipeline scope, and avoid broad "
                "implementation decisions unless explicitly asked."
            ),
            advanced=False,
        ),
        MessageTextInput(
