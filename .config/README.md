...
Every Terraform tfvars file, shared component tfvars, `terraform/minio.backend.hcl`, and live `docker/*.env` / `docker/*.yaml` under this tree must start with a **first-line** tag:
...
Export **`CONFIG_DIR`** in the environment only when this host keeps the site config tree somewhere other than **`<repo>/.config`** (for example a CI bind-mount). The canonical layout is still **`$CONFIG_DIR/terraform/components/...`**.

## Git

The repo **`.gitignore` ignores `.config/*` except `README.md`, `docker/README.md`, `docker/*.env.example`, `docker/*.yaml.example`, `scripts/*.env.example`**, so live secrets and site tfvars are not committed accidentally. Do not force-add tfvars, `docker/*.env`, `docker/*.yaml`, or private keys.
